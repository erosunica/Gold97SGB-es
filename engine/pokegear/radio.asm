...
; If Team Rocket is not occupying the radio tower, we don't need to be here.
	...
	jr z, .ok
; If we're in SWIslands, we don't need to be here.
	call IsInNihon
	and a
	jr nz, .ok
...
.sample
	...
	; Search the NihonGrassWildMons array for the chosen map.
	ld hl, NihonGrassWildMons
.loop
	ld a, BANK(NihonGrassWildMons)
	call GetFarByte
	cp -1
	jr z, .overflow
	inc hl
...
PeoplePlaces4: ; People
	...
	jr nz, .ok
	ld hl, PnP_HiddenPeople_BeatSWIslands
.ok
	pop af
	ld c, a
	ld de, 1
